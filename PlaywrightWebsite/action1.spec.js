...
test('Verify Header Text', async ({ page }) => {
  ...
  async function Action1(page) {
          ...
          const language = page.getByRole('button', {name:'Node.js'});
          const community = page.getByRole('link', {name:'Community'});
          const Python = page.getByRole('link',{name:'Python'});
          const Java = page.getByRole('link', {name:'Java', exact:true});
          const Net = page.getByRole('link',{name:'.NET'});
          const search = page.getByLabel('Search (Ctrl+K)');
